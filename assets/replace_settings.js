...
const panelSettingsHtml = `
            <!-- Panel: panel-settings (Profile Details) -->
...
['client-dashboard.html', 'admin-dashboard.html'].forEach(file => {
    ...
    const startIndex = content.indexOf('<!-- Panel: panel-settings -->');
    const endIndex = content.indexOf('<!-- Panel: panel-support -->');
    
    if (startIndex !== -1 && endIndex !== -1) {
        content = content.substring(0, startIndex) + panelSettingsHtml + content.substring(endIndex);
        fs.writeFileSync(file, content);
        console.log('Successfully replaced panel-settings in ' + file);
    } else {
        console.log('Could not find start/end index in ' + file);
    }
});
