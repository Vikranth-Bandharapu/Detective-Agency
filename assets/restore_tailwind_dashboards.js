...
async function extract() {
    ...
    for await (const line of rl) {
        ...
        if (obj.step_index >= 1269) break; // Stop before add_settings.js
    ...
    if (bestClientCode) {
        ...
        console.log('Restored client-dashboard.html to before add_settings.js (size: ' + bestClientCode.length + ')');
    }
    if (bestAdminCode) {
        fs.writeFileSync('C:\\Users\\admin\\Desktop\\Detective agency\\admin-dashboard.html', bestAdminCode);
        console.log('Restored admin-dashboard.html to before add_settings.js (size: ' + bestAdminCode.length + ')');
    }
}
