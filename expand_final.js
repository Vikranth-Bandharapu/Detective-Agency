...
function wrapPanel(comment, id, title, desc, innerHtml) {
...
["client-dashboard.html", "admin-dashboard.html"].forEach(file => {
    ...
    content = content.replace(/<!-- Panel: Cases -->[\s\S]*?(?=<!-- Panel: Intel Reports -->)/, wrapPanel("Cases", "panel-cases", "Active Case Files", "Classified ongoing investigations.", getCases()));
    content = content.replace(/<!-- Panel: Intel Reports -->[\s\S]*?(?=<!-- Panel: panel-operatives -->)/, wrapPanel("Intel Reports", "panel-intel", "Global Intelligence", "Intercepted data and surveillance feeds.", getIntel()));
    content = content.replace(/<!-- Panel: panel-operatives -->[\s\S]*?(?=<!-- Panel: panel-analytics -->)/, wrapPanel("panel-operatives", "panel-operatives", "Agency Roster", "Manage field agents and analysts.", getOps()));
    content = content.replace(/<!-- Panel: panel-analytics -->[\s\S]*?(?=<!-- Panel: panel-comms -->)/, wrapPanel("panel-analytics", "panel-analytics", "Agency Analytics", "Performance metrics and success rates.", getAnalytics()));
    content = content.replace(/<!-- Panel: panel-comms -->[\s\S]*?(?=<!-- Panel: panel-settings)/, wrapPanel("panel-comms", "panel-comms", "Secure Comms", "Encrypted messaging intercept logs.", getComms()));
    content = content.replace(/<!-- Panel: panel-support -->[\s\S]*?(?=<\/div>\s*<\/main>)/, wrapPanel("panel-support", "panel-support", "Technical Support", "Hardware requisitions and IT tickets.", getSupport()));
